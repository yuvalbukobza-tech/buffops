...
jsx = jsx.replace(
    ...
    '          <button onClick={()=>{\n'
    '            const data = {products,allocs,orders,transactions,raffles,raffleCountries,budgetExtras,appUsers,exportedAt:new Date().toISOString()};\n'
    '            const blob = new Blob([JSON.stringify(data,null,2)],{type:"application/json"});\n'
    '            const a = document.createElement("a"); a.href=URL.createObjectURL(blob);\n'
    '            a.download="buffops-backup-"+new Date().toISOString().slice(0,10)+".json"; a.click();\n'
    '          }} style={{marginLeft:12,background:"#1a1a1a",border:"1px solid #2a2a2a",borderRadius:7,color:"#888",padding:"5px 12px",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>⬇ Export Backup</button>\n'
    '        </div>\n'
    '      </div>\n'
    '      <div style={{padding:36}}>'
)
