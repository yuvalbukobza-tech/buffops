...
jsx = jsx.replace(
    ...
    '\n'
    '  // Pre-auth: load appUsers from Sheets so login works on any device\n'
    '  useEffect(()=>{\n'
    '    fetch(SCRIPT_URL)\n'
    '      .then(r=>r.json())\n'
    '      .then(data=>{\n'
    ...
    '          </div>\n'
    '          <button type="submit" disabled={!usersReady} style={{width:"100%",background:"#c8ff00",color:"#0a0a0a",border:"none",\n'
    '            borderRadius:8,padding:"11px",fontSize:14,fontWeight:800,cursor:usersReady?"pointer":"not-allowed",fontFamily:"inherit",letterSpacing:"0.02em",opacity:usersReady?1:0.6}}>\n'
